...
`desktop`-запуск не привязан только к macOS arm64: базовый профиль `fake` должен подниматься и на Windows x64. Для live-профилей с BLE/ANT+ всё ещё зависят от драйверов, железа и конкретной платформы.
