...
Проблема в другом: если каждый processor будет сам заново реализовывать буферы, отбор потоков, условия запуска и хранение последних состояний, код быстро развалится на десятки несовместимых локальных реализаций.
...
Дополнительная фиксация терминологии:

- если речь идёт про runtime identity, `processor-kit` оперирует только термином `stream` и полем `streamId`;
- термин `channel` для shared signal-контракта в `v1` не используется.
...
- автоматическое выравнивание по времени между несколькими потоками;
...
Обработчик должен уметь декларативно описывать сигнальные входы по `streamId`.
...
signals: {
  emg: {
    streamId: 'trigno.emg.vastus_lateralis',
...
- идентификатором сигнала в `v1` является `streamId`;
...
- фильтры по `streamId` там, где это применимо;
...
### Нет автоматического выравнивания потоков
