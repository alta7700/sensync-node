...
- `App.tsx` содержит runtime singleton, рекурсивный renderer `row/column/widget`, локальные modal forms, профильные inline-блоки `veloerg` для лактата/мощности, summary-строку перед графиками, toast-уведомления и ECharts/canvas графики, включая history-режим с data-zoom для viewer-профилей; в summary лактат выводится одной строкой `{время} - {значение}` по последней точке потока, а submit-валидация не пропускает лактат `>= 100` и мощность `>= 1000`.
