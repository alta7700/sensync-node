...
- `App.tsx` содержит runtime singleton, рекурсивный renderer `row/column/widget`, локальные modal forms, профильный inline-блок `veloerg` для мощности, summary-строку перед графиками, toast-уведомления и ECharts/canvas графики, включая history-режим с data-zoom для viewer-профилей; status-виджеты умеют показывать именованные поля, а ECharts-графики могут рисовать vertical marker по flag-значению; submit-валидация не пропускает мощность `>= 1000`.
