...
- Для `veloerg` renderer дополнительно рисует отдельные inline-блоки лактата и мощности, автопилот мощности с ручным режимом и summary-строку с текущими метриками перед графиками; лактат в summary показывается как `{время} - {значение}` по последней точке потока, а UI не отправляет лактат `>= 100` и мощность `>= 1000`.
