...
- В `veloerg` тот же `trigno-adapter` работает в paired mode: UI передаёт `host`, `vlSensorSlot` и `rfSensorSlot`, а один TCP-session публикует `trigno.vl.avanti*` и `trigno.rf.avanti*`.
