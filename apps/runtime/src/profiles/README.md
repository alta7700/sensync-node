...
- UI схемы `veloerg` рисуют отдельный inline-блок `power`, summary-строку с живыми метриками и ручной/автопилотный режим мощности.
...
- `veloerg-replay` — replay-профиль для HDF5 записей `veloerg`:
  ...
  - replay ограничен stream'ами `moxy.*`, `zephyr.*`, raw `trigno.*` и `power.label`, чтобы UI повторял именно `veloerg`-контракт записи.
- `veloerg-viewer` — viewer-профиль для HDF5 записей `veloerg`:
  - поднимает `hdf5-viewer-adapter` и `ui-gateway`;
  - не требует `SENSYNC2_HDF5_SIMULATION_FILE` на старте, потому что путь выбирается через `adapter.connect.request.formData.filePath`;
  - viewer ограничен теми же stream'ами `moxy.*`, `zephyr.*`, raw `trigno.*` и `power.label`, но отдает историю целиком для интерактивного просмотра графиков.
