...
- `fake.ts`, `fake-hdf5-simulation.ts`, `veloerg.ts`, `veloerg-replay.ts`, `veloerg-viewer.ts`, `veloerg-final-viewer.ts`, `pedaling-emg-test.ts`, `pedaling-emg-replay.ts`, `pedaling-emg-viewer.ts` — отдельные модули профилей.
...
- `veloerg` также поднимает generic `label-generator-adapter` для sparse-stream'а `power.label`.
- UI схемы `veloerg` рисуют отдельный inline-блок `power`, summary-строку с живыми метриками и две отдельные пары raw-графиков `VL EMG / VL Gyro` и `RF EMG / RF Gyro`.
- `veloerg` использует paired Trigno-connect contract: `adapter.connect.request.formData` должен содержать `host`, `vlSensorSlot` и `rfSensorSlot`.
- `pedaling-emg-processor` временно исключён из `veloerg`, потому что live `Trigno` поток переполнял его mailbox; вместо этого профиль пишет и показывает только raw парные потоки `trigno.vl.avanti*` и `trigno.rf.avanti*`.
...
- `veloerg-replay` — replay-профиль для HDF5 записей `veloerg`:
  ...
  - replay ограничен stream'ами `moxy.*`, `zephyr.*`, `trigno.vl.avanti*`, `trigno.rf.avanti*` и `power.label`, чтобы UI повторял именно `veloerg`-контракт записи;
  - `requireAllStreamIds = true` делает этот контракт жёстким: старые single-sensor `veloerg` файлы с `trigno.avanti*` считаются неподдерживаемыми.
- `veloerg-viewer` — viewer-профиль для HDF5 записей `veloerg`:
  - поднимает `hdf5-viewer-adapter` и `ui-gateway`;
  - не требует `SENSYNC2_HDF5_SIMULATION_FILE` на старте, потому что путь выбирается через `adapter.connect.request.formData.filePath`;
  - viewer ограничен теми же stream'ами `moxy.*`, `zephyr.*`, `trigno.vl.avanti*`, `trigno.rf.avanti*` и `power.label`, но отдает историю целиком для интерактивного просмотра графиков;
  - `requireAllStreamIds = true` здесь тоже обязателен, поэтому viewer не маскирует legacy single-sensor `veloerg` файл под новую схему.
- `veloerg-final-viewer` — отдельный viewer-профиль для HDF5 записей `veloerg` с расширенным `train.red` контрактом:
  - поднимает `hdf5-viewer-adapter` и `ui-gateway`;
  - не требует `SENSYNC2_HDF5_SIMULATION_FILE` на старте, потому что путь выбирается через `adapter.connect.request.formData.filePath`;
  - вместо старого `moxy.thb` требует `train.red.smo2`, `train.red.hbdiff`, `lactate` и пять `*.unfiltered` потоков, чтобы UI показывал filtered/unfiltered графики, отдельный график лактата и не подменял отсутствующий канал;
  - пробрасывает file-level scalar metadata из root attrs HDF5 в viewer UI, поэтому `veloerg-final-viewer` показывает отдельный статус-блок с параметрами испытуемого и временем остановки, а также отдельный блок `LT2`;
  - `stop_time_sec`, если он есть в metadata файла, materialize'ится в UI как вертикальная пунктирная линия на всех графиках этого профиля;
  - `lt2_refined_time_sec`, если он есть в metadata файла, materialize'ится в UI как вторая вертикальная пунктирная линия другого цвета на всех графиках этого профиля;
  - `requireAllStreamIds = true` здесь тоже обязателен, поэтому профиль явно отбрасывает файлы без полного набора `train.red` и парных `Trigno` stream'ов.
