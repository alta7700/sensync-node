...
- `veloerg` также включает recorder-driven `timelineReset`:
  ...
  - participants: `ui-gateway`, live adapters, derived HR processor и recorder как явно описанный reset-aware subset;
  - `zephyr-bioharness-3-adapter`, `hr-from-rr-processor` и `hdf5-recorder` получают `required` через config-dependent manifest;
