...
- UI схемы `veloerg` рисуют отдельные inline-блоки `lactate` и `power`, summary-строку с живыми метриками и ручной/автопилотный режим мощности.
- В `veloerg` временно разрешён `timeline.reset` от `external-ui`, чтобы можно было тестировать reset-flow без активной записи.
