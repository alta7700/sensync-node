...
- Lifecycle, scan cache, reconnect timer и irregular emit адаптера теперь собраны на `@sensync2/plugin-kit`.
...
- После `reset/reconnect/sequence gap` RR-поток заново якорится по локальному времени первого пакета; более ранние точки из этого пакета обрезаются по старту текущего timeline, чтобы один большой интервал не уводил весь UI далеко вперёд.

## Взаимодействие

- Использует `@sensync2/plugin-kit`, `@sensync2/core`, `@sensync2/plugin-sdk` и `@abandonware/noble`.
