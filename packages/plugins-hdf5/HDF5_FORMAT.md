...
Каждый канал хранится в группе:

- `/channels/<channelId>`

## 3. Обязательные attrs группы канала
...
## 4. Необязательные attrs группы канала
...
Внутри `/channels/<channelId>` должны быть datasets:
...
- если в config передан `channelIds`, adapter проигрывает только их и игнорирует остальные группы файла;
- чтение идёт только вперёд;
- курсоры по каналам сдвигаются по окнам `batchMs`;
- в один цикл симуляции каждый канал может отдать максимум один `signal.batch`;
